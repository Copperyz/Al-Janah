    ...
    if (
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          var value =
            parseFloat(response) > 0 ? parseFloat(response) : $('#addShipmentItemForm').find('[name="price"]').val();
    ...
    if (parcelTypeId == 1 && height && width && weight && length && parcelTypeId && goodTypeId && trip_route_id) {
      ...
      $.ajax({
        ...
        success: function (response) {
          // Update the UI with the calculated price
          var value =
            parseFloat(response) > 0 ? parseFloat(response) : $('#editShipmentItemForm').find('[name="price"]').val();
