      ...
      <div class="modal-body">
        ...
        <div class="row">
          ...
          <div class="d-flex justify-content-between bg-lighter p-2 mb-3">
            ...
            <p class="fw-medium mb-0" id="shipmentPrice">{{$shipment->shipmentPrice}} {{__('LYD')}}</p>
          </div>
          <div class="d-flex justify-content-between bg-lighter p-2 mb-3">
            <p class="mb-0">{{__('Packages cost')}}:</p>
            <p class="fw-medium mb-0">{{$shipment->amount}} {{__('LYD')}}</p>
          </div>
          <form id="addPaymentForm" class="g-3" action="{{ route('payments.store') }}" method="POST" onsubmit="return false">
            <div class="mb-3">
              <label class="form-label" for="invoiceAmount">{{__('Total Amount')}}</label>
              <div class="input-group">
                <span class="input-group-text">{{__('LYD')}}</span>
                <input type="text" id="invoiceAmount" name="invoiceAmount" class="form-control invoice-amount" placeholder="{{$shipment->amount + $shipment->shipmentPrice}}" value="{{$shipment->amount + $shipment->shipmentPrice}}" disabled />
              </div>
            </div>
            <div class="mb-3">
            ...
            <div class="row d-none my-3" id="couponInput">
              <div class="input-group input-group mb-3">
                <input type="text" name="couponCode" class="form-control " placeholder="{{__('Coupon Code')}}" aria-describedby="apply coupon">
